...
# ==========================================
# 2. 詳細用語解説 (Detailed Explanation)
# ==========================================
def draw_detailed_explanation():
    st.markdown("---")
    st.header("📖 臨床薬理・血液浄化 用語事典")
    
    with st.expander("1. 分布容積 V1 (中心室) と V2 (末梢室) の本質", expanded=False):
        st.markdown("""
    ...
    with st.expander("2. KoA (総括物質移動係数) と透析効率", expanded=False):
        st.markdown("""
        **イメージ: 「ふるいの目の粗さと面積」**
    ...
    with st.expander("3. リバウンド (Rebound) の発生機序", expanded=False):
        st.markdown("""
        **現象: 「透析が終わったのに、また濃度が上がる」**
        
        1.  **透析中:** $V_1$（血液）の薬物が急速に除去され、濃度が下がります。一方、$V_2$（組織）の濃度は高いままです。
        2.  **不均衡:** 「組織($V_2$) > 血液($V_1$)」という極端な濃度差（勾配）が生まれます。
        3.  **移動 (Redistribution):** 濃度差に従って、薬物は組織から血液へ移動し続けますが、組織間移行速度($Q$)には限界があるため、透析中の供給は追いつきません。
        4.  **透析終了:** 除去ストップ。しかし組織からの流入は続くため、血中濃度が再び上昇します。
        
        > **警告:** リチウム中毒などで「透析で安全域まで下がった」と判断して帰宅させ、数時間後に意識障害が再燃するのはこのためです。
        """)

    with st.expander("4. 腎機能・中毒と消失半減期 (T1/2)", expanded=False):
        st.markdown("""
        **なぜ中毒時に半減期が伸びるのか？**
        
        * **腎排泄型 (アシクロビル、リチウム):** * 排水溝（腎臓）が詰まっている状態です。透析（ポンプによる汲み出し）をしない限り、水位（濃度）は下がりません。
        * **肝代謝型 (カフェイン、テオフィリン):** * **代謝飽和 (Saturation):** 通常、肝臓の酵素は余裕を持って働いていますが、大量の薬物が来ると「処理待ち行列」ができます（ミカエリス・メンテン式における0次速度過程への移行）。
            * これにより、見かけ上の半減期が**数倍〜数十倍**に延長します。これをシミュレートするために、中毒時は長い半減期を入力する必要があります。
        """)
...
# --- パラメータと閾値定義 ---
# thresholds: グラフに線を引く危険水域 (単位: mg/L or µg/mL)
default_params = {
    'Caffeine': {
    ...
    'Acyclovir': {
        ...
        'thresholds': {'Neurotoxicity (>50)': 50} # 腎不全時の目安
    },
    'Carbamazepine': {
    ...
    'Valproic Acid': {
        ...
        # HD適応はガイドラインにより異なるが、中毒症状が重篤化するラインとして設定
    },
    'Methanol': {
        'V1': 0.6, 'V2': 0.1, 'Q': 0.8, 'T1/2': 40.0, 'KoA': 900, 'dose': 30000,
        'thresholds': {'Toxic (>200)': 200, 'HD Indication (>500)': 500} # 20mg/dL, 50mg/dL
    },
    'Lithium': {
        'V1': 0.3, 'V2': 0.6, 'Q': 0.15, 'T1/2': 40.0, 'KoA': 850, 'dose': 4000,
        'thresholds': {'Toxic (>1.5 mEq/L approx 10mg/L)': 10.5, 'Severe (>2.5 mEq/L approx 17.5mg/L)': 17.5}
        # Note: 1 mEq/L Li = 6.94 mg/L. 
    },
    'Custom': {
...
with st.sidebar.expander("薬剤パラメータ詳細設定", expanded=True):
    ...
    with col_k1:
        t_half = st.number_input("半減期 (時間)", value=float(p['T1/2']))
    with col_k2:
...
if st.button("シミュレーション実行", type="primary"):
    ...
    with col1:
        ...
        # Danger Lines (Toxic Thresholds)
        # グラフが見にくくならないよう、薄い赤線で描画
